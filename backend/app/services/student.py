...
async def create_student(db: AsyncSession, tenant_id: str, data: dict):
    # Convert date strings → date objects before DB insert
    for field in ("date_of_birth", "joined_at", "left_at"):
        val = data.get(field)
