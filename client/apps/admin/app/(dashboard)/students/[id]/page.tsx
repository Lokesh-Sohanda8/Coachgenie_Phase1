...
//   const [student,      setStudent]      = useState<any>(store.students.find(s => s.id === id) ?? null);
...
//   const batches    = store.batches.filter(b => student.batchIds?.includes(b.id));
...
//                       <p className="text-xs text-muted-foreground">{b.teacher} · {b.room}</p>
//                       <p className="text-xs text-muted-foreground">
//                         {b.schedule.map((s: any) => `${s.day} ${s.time}`).join(" · ")}
//                       </p>
//                     </div>
//                     <span className={cn(
...
export default function StudentProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // ── fetch enrolled batches directly from API ───────────────
  // Uses GET /batches/ (all batches) then filters by student_id from batch_students,
  // OR if you have GET /students/{id}/batches use that instead.
  // Currently fetches all batches and finds ones where this student is enrolled
  // by calling GET /batches/{batch_id}/students for each — but that's N+1.
  // Best approach: fetch all batches, then check enrollment via batch_students.
  // Since backend has GET /batches/{batch_id}/students, we fetch all batches
  // and check which ones contain this student.

  // useEffect(() => {
  //   async function loadBatches() {
  //     try {
  //       // Step 1: get all batches
  //       const res = await fetch(`${API}/batches/?limit=100`, { headers: authHeaders() });
  //       if (!res.ok) return;
  //       const json = await res.json();
  //       const allBatches: any[] = Array.isArray(json)
  //         ? json
  //         : (json.data ?? json.items ?? json.batches ?? []);

  //       // Step 2: filter batches where student_ids includes this student
  //       // The BatchOut schema includes student_ids (enriched by _attach_student_ids)
  //       const enrolled = allBatches.filter(b => {
  //         const ids: string[] = b.student_ids ?? [];
  //         return ids.includes(id);
  //       });

  //       setBatches(enrolled.map(b => ({
  //         id:       String(b.id),
  //         name:     b.name          ?? "",
  ...
  //   }
  //   loadBatches();
  // }, [id, showEnroll]); // ← re-fetch after EnrollmentDialog closes
  useEffect(() => {
  async function loadBatches() {
    try {
