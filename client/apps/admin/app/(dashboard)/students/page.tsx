...
//       // 2. Enrich subjects + fees in parallel for each student
//       const enriched = await Promise.all(
//         admissionStudents.map(async (s) => {
//           // subjects
//           let subjects = s.subjects ?? [];
//           if (!subjects.length && s.admission_id) {
//             try {
...
//             const invoices: any[] = fJson.data ?? [];
//             fees = computeFees(invoices);
//           } catch {}

//           return { ...s, subjects, _fees: fees };
//         })
//       );
...
//       updateStudent(editStudent.id, mapStudent(updated, editStudent.fees)); // keep existing fees
...
export default function StudentsPage() {
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      ...
      const enriched = await Promise.all(
        admissionStudents.map(async (s) => {
          // fetch fees — handle array or wrapped response
          let fees = { total: 0, paid: 0, due: 0 };
          try {
            const fRes  = await fetch(`${API}/fees/student/${s.id}`, { headers: authHeaders() });
            const fJson = fRes.ok ? await fRes.json() : {};
