...
//     id:         String(raw.id),
//     name:       raw.name         ?? "",
//     subject:    raw.target_exam  ?? raw.code ?? "",   // no subject field in BatchOut — use target_exam
...
//           : (err.detail ?? "Failed to create batch");
//         toast.error(detail);
//         return;
//       }
...
//         const detail = Array.isArray(err.detail)
//           ? err.detail.map((e: any) => `${e.loc?.slice(-1)[0]}: ${e.msg}`).join(", ")
//           : (err.detail ?? "Failed to update batch");
//         toast.error(detail);
//         return;
//       }
...
//     });
//     if (!res.ok) { toast.error("Failed to deactivate batch"); return; }
...
//         <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
//           {filtered.map(batch => {
//             const syllabusCompleted = batch.syllabus.filter(t => t.completed).length;
