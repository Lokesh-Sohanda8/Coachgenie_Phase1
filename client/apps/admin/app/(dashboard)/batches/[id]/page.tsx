...
// import { ArrowLeft, Users, Calendar, BookOpen, X } from "lucide-react";
...
// // ✅ FIXED: Read TENANT inside the function so it's never stale/undefined
// function authHeaders(): HeadersInit {
//   const token  = sessionStorage.getItem("access_token");
//   const tenant = process.env.NEXT_PUBLIC_TENANT_SUBDOMAIN ?? "demo"; // ✅ moved inside
//   return {
...
// // ── Helper: fetch enrolled students for a batch ───────────────
// async function fetchBatchStudents(batchId: string) {
//   const sRes  = await fetch(`${API}/batches/${batchId}/students`, { headers: authHeaders() }); // ✅ FIXED endpoint
//   if (!sRes.ok) throw new Error("Failed to fetch students");
//   const sJson = await sRes.json();
//   const enrolled: any[] = Array.isArray(sJson) ? sJson : (sJson.data ?? []);
...
//   // ── Fetch batch + students + classes ──────────────────────
//   useEffect(() => {
//     async function load() {
//       setLoading(true);
//       setError(null);
//       try {
//         // 1. Fetch batch if not already in store
...
//         // 3. ✅ FIXED: Fetch enrolled students via dedicated batch endpoint
...
//       // ✅ FIXED: Re-fetch from correct endpoint
//       const enrolled = await fetchBatchStudents(id);
//       setStudents(enrolled);
//     } catch (err: any) {
...
//           { label: "Syllabus", value: `${syllabusProgress}%`,                icon: BookOpen },
//           { label: "Started",  value: batch.startDate
//               ? format(new Date(batch.startDate), "dd MMM")
//               : "—",                                                          icon: Calendar },
...
//                     {c.scheduled_at
//                       ? format(new Date(c.scheduled_at), "dd MMM, h:mm a")
//                       : "—"}
...
//                   {c.status !== "completed" && (
//                     <button
//                       onClick={() => handleMarkClassDone(String(c.id))}
