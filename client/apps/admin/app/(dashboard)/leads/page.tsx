...
//       fetchLeads(); // roll back by re-fetching
//     }
//   }
...
//               className={cn(
//                 "rounded-full px-3 py-1 text-xs font-medium border transition-colors",
//                 stageFilter === s ? `${cfg.color} ${cfg.bg} ${cfg.border}` : "hover:bg-accent"
//               )}
//             >
//               {cfg.label} ({count})
//             </button>
...
//           <div className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 w-full max-w-2xl rounded-2xl border bg-background shadow-2xl">
...
export default function LeadsPage() {
  ...
  const fetchBatches = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) return; // silent — batches are optional
      const json = await res.json();
      const raw: any[] = Array.isArray(json) ? json : (json.data ?? json.items ?? []);
      setBatches(
        raw.map((b) => ({
      ...
      // silent failure — form still usable without batches
    } finally {
  ...
  // ── Create — persists all fields including standard / board / batch ────────
  async function handleCreate(data: LeadFormValues) {
    try {
      const res = await fetch(`${API}/leads/`, {
        method:  "POST",
        ...
        body: JSON.stringify({
          ...
          interested_course:     data.subject,
          notes:                 data.notes ?? "",
          // ── new fields sent to DB ─────────────────────────────────────────
          board_name:            data.boardName ?? "",
          batch_id:              data.batchId   || null,   // null if empty string
        }),
      });
  ...
  // ── Stage change (kanban drag) ─────────────────────────────────────────────
  async function handleStageChange(id: string, stage: LeadStage) {
    updateStage(id, stage); // optimistic
    try {
      const res = await fetch(`${API}/leads/${id}`, {
        method:  "PATCH",
        headers: authHeaders(),
