...
//   FULL:    { label: "Full Payment",   className: "bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300" },
//   PARTIAL: { label: "Partially Paid", className: "bg-amber-100 text-amber-700 dark:bg-amber-950 dark:text-amber-300" },
//   PENDING: { label: "Pending",        className: "bg-red-100 text-red-700 dark:bg-red-950 dark:text-red-300" },
// };
...
//   remaining: number, count: number, dates: string[]
// ): InstallmentSchedule[] {
//   if (!count) return [];
...
// /** Read auth from Zustand persist key in localStorage */
...
//     payment.installmentSchedule.map(s => s.dueDate ?? "")
//   );
...
//   const instAmt   = hasInst && instCount > 0 && remaining > 0
//     ? Math.ceil(remaining / instCount) : 0;
...
//     onSave({
//       totalFee: total, amountPaid: paid, remaining,
//       paymentStatus: payStatus, dateOfPayment: pdate, modeOfPayment: pmode,
//       hasInstallments: hasInst, numberOfInstallments: hasInst ? instCount : 0,
//       installmentAmount: instAmt, installmentSchedule: schedule, notes,
//     });
//   }
...
//               <input type="number" min="0" value={totalFee}
//                 onChange={e => setTotalFee(e.target.value)} className={inputCls()} />
//             </Field>
...
//               PAYMENT_STATUS_CONFIG[payStatus].className)}>
//               {PAYMENT_STATUS_CONFIG[payStatus].label}
//             </span>
//           </div>

//           <div className="grid grid-cols-2 gap-4">
//             <Field label="Date of Payment">
//               <input type="date" value={pdate}
//                 onChange={e => setPdate(e.target.value)} className={inputCls()} />
//             </Field>
//             <Field label="Mode of Payment">
//               <select value={pmode}
//                 onChange={e => setPmode(e.target.value as PaymentMode)} className={inputCls()}>
...
//             <input type="checkbox" checked={hasInst}
//               onChange={e => {
//                 setHasInst(e.target.checked);
//                 if (e.target.checked)
//                   setInstDates(Array.from({ length: instCount }, (_, i) => instDates[i] ?? ""));
//               }}
...
//                   <select value={instCount}
//                     onChange={e => handleInstCount(Number(e.target.value))} className={inputCls()}>
...
//                         Installment {i + 1}
//                       </span>
//                       <input type="date" value={instDates[i] ?? ""}
//                         onChange={e => {
//                           const d = [...instDates]; d[i] = e.target.value;
//                           setInstDates(d);
//                         }}
...
//   // ── Patch helper — sends PATCH to API then re-fetches ─────────────────────
...
//       }
//       await fetchAdmission(); // re-fetch to get fresh data
//     } catch (err: any) {
...
//           className="text-sm text-primary hover:underline">← Back</button>
//       </div>
//     );
//   }

//   // ── Resolve fields — accept both camelCase and snake_case ─────────────────
...
//   const subjects     = adm.subjects?.length
//                          ? adm.subjects
//                          : adm.applied_course
//                          ? [adm.applied_course]
//                          : [];
...
//     totalFee:             feeAmount,
//     amountPaid:           feePaid,
//     remaining:            Math.max(0, feeAmount - feePaid),
//     paymentStatus:        derivePaymentStatus(feePaid, feeAmount),
//     dateOfPayment:        createdAt?.split("T")[0] ?? "",
...
//   const feePercent   = feeAmount > 0
//     ? Math.min(100, Math.round((feePaid / feeAmount) * 100))
//     : 0;
...
//     const updatedDocs = adm!.documents.map((d) =>
//       d.name === name ? { ...d, submitted: !d.submitted } : d
//     );
...
//       status:     next,
//       enrolledAt: next === "CONFIRMED" ? new Date().toISOString() : undefined,
//     });
...
//       status:
//         updated.paymentStatus === "FULL"    ? "CONFIRMED"   :
//         updated.paymentStatus === "PARTIAL" ? "FEE_PENDING" :
//         adm!.status,
//     });
...
//     const updatedSchedule = payment.installmentSchedule.map((s, i) =>
//       i === idx ? { ...s, paid: true } : s
//     );
//     const instPaidTotal = updatedSchedule
//       .filter(s => s.paid)
...
//       remaining:           Math.max(0, payment.totalFee - newAmountPaid),
//       paymentStatus:       derivePaymentStatus(newAmountPaid, payment.totalFee),
...
//               {(adm.documents ?? []).filter((d) => d.submitted).length}/
//               {(adm.documents ?? []).length} submitted
//             </span>
//           </div>
...
//                     doc.submitted && "line-through text-muted-foreground")}>
//                     {doc.name}
//                   </span>
//                   {doc.required
...
//             <p className="text-xs text-emerald-600 bg-emerald-50 dark:bg-emerald-950 rounded-lg px-3 py-2">
...
//             <span className={cn("text-xs font-medium px-2.5 py-0.5 rounded-full", psCfg.className)}>
//               {psCfg.label}
//             </span>
//             {payment.modeOfPayment && (
//               <span className="text-xs text-muted-foreground">
//                 via {PAYMENT_MODE_LABELS[payment.modeOfPayment]}
//               </span>
//             )}
//             {payment.dateOfPayment && (
//               <span className="text-xs text-muted-foreground">
//                 · {fmtDate(payment.dateOfPayment)}
//               </span>
//             )}
//           </div>
...
//                   feePercent === 100 ? "bg-emerald-500"
//                   : feePercent >= 50  ? "bg-blue-500"
//                   : "bg-amber-500")}
...
//           {/* Dates — all guarded with fmtDate so no Invalid Date crashes */}
...
//               <div className="flex justify-between">
//                 <span>Enrolled</span>
//                 <span className="text-emerald-600">{fmtDate(approvedAt)}</span>
//               </div>
//             )}
//           </div>
//         </div>
...
//             </h3>
//             <span className="text-xs text-muted-foreground">
//               {payment.installmentSchedule.filter(s => s.paid).length} /{" "}
//               {payment.numberOfInstallments} paid
//             </span>
//           </div>
...
//                   "rounded-lg border p-3 flex items-start justify-between gap-3 transition-all",
//                   inst.paid
...
//                       inst.paid ? "text-emerald-600"
//                       : isOverdue ? "text-red-500"
//                       : "text-muted-foreground")}>
//                       {fmtDate(dueDate, "No date set")}
//                       {isOverdue && " · Overdue"}
//                     </p>
...
//                       disabled={saving}
//                       className={cn(
//                         "shrink-0 rounded-lg px-2.5 py-1 text-xs font-medium transition-colors disabled:opacity-50",
//                         isOverdue
...
//                 {fmt(payment.installmentSchedule.filter(s => s.paid).reduce((sum, s) => sum + s.amount, 0))}
//               </span>
//             </span>
//             <span>
//               Remaining:{" "}
